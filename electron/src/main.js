...
const { connectTunnel } = require('./tunnel');
...
ipcMain.handle('portshare:start-tunnel', async (event, config) => {
  if (activeTunnel) {
    activeTunnel.close();
  }
  ...
  activeTunnel = connectTunnel({
    ...
    onStateChange: (state, msg) => {
      if (mainWindow) {
        mainWindow.webContents.send('portshare:tunnel-state', state, msg);
      }
    },
    onLogEntry: (entry) => {
      if (mainWindow) {
        mainWindow.webContents.send('portshare:tunnel-log', entry);
      }
    }
  });
  return true;
...
ipcMain.handle('portshare:stop-tunnel', async () => {
  if (activeTunnel) {
    activeTunnel.close();
    activeTunnel = null;
  }
...
ipcMain.handle('portshare:update-tray', async (event, tunnels) => {
  if (!tray) return false;
  const menuItems = [];
  menuItems.push({ label: 'Show PortShare', click: () => { mainWindow?.show(); mainWindow?.focus(); } });
  menuItems.push({ label: 'Hide PortShare', click: () => mainWindow?.hide() });
  
  if (tunnels && tunnels.length > 0) {
    menuItems.push({ type: 'separator' });
    menuItems.push({ label: 'Saved Tunnels', enabled: false });
    tunnels.forEach(t => {
      const isRunning = activeTunnel && currentConfig && currentConfig.port === t.port;
      menuItems.push({
        label: `${t.subdomain} (:${t.port})`,
        icon: isRunning ? nativeImage.createFromPath(path.join(__dirname, '../assets/green-dot.png')) : undefined,
        submenu: [
          { label: 'Copy URL', click: () => { 
            const domain = t.customDomain ? t.customDomain : `${t.subdomain}.portshare.dev`; 
            require('electron').clipboard.writeText(`https://${domain}`); 
          }},
        ]
      });
    });
...
ipcMain.handle('portshare:set-autostart', async (event, enabled) => {
